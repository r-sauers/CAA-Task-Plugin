...
class Util {
     ...
     */
    public static parseID(id_str: string): number {
...
class Column {
    ...
    /**
     * The text in the header of the column
     * @since 1.0.0
     * @type {string}
     */
    ...
    /**
     * Callback used to draw information in every cell of the column using some row datum.
     * @since 1.0.0
     * @type {(row_datum: any) => string | HTMLElement}
     */
    private draw_callback: (row_datum: any) => string | HTMLElement;
     ...
     **/
    constructor(header: string, draw_callback: (row_datum: any) => string | HTMLElement) {
        this.header = header;
     ...
     */
    public create_cell(row_datum: any): HTMLTableCellElement {
...
class Table {
    ...
    /**
     * Used to draw header/cells of each column.
     * @since 1.0.0
     * @type {Column[]}
     */
    ...
    /**
     * If defined, this callback is called on every row after it is made.
     * @since 1.0.0
     * @type {(row_datum: any, row: HTMLTableRowElement) => void) | undefined}
     */
    private row_callback: ((row_datum: any, row: HTMLTableRowElement) => void) | undefined;
     ...
     */
    public add_column(header: string, draw_callback: (row_datum: any) => HTMLElement | string) {
     ...
     */
    public set_row_callback(row_callback: ((row_datum: any, row: HTMLTableRowElement) => void) | undefined) {
     ...
     */
    public draw(container: HTMLElement, data: any[]) {
        ...
        for (let column of this.columns) {
            $("<th>").appendTo($tr_head).html(column.get_header());
        }
     ...
     */
    private create_row(row_datum: any) {
...
class Form_Field {
    ...
    /**
     * The label element before the field or null if there is no label.
     * 
     * @since 1.0.0
    ...
    /**
     * The field element. This could be an input element or something like a textarea element.
     * 
     * @since 1.0.0
     * @type {HTMLElement}
     */
    ...
    constructor(
        ...
        const default_config = (field_config['tag_name']) ? default_config_tag : default_config_input;
     ...
     */
    private create_tag_element(name: string, field_config): HTMLElement {
        const $tag = $(`<${field_config['tag_name']}>${field_config['default']}</${field_config['tag_name']}>`);
        $tag.attr("name", name);
     ...
     * @returns {HTMLElement} an html element with the tag specified by field_config
     */
    private create_input_element(name: string, field_config):HTMLInputElement {
        const $field = $("<input/>");
     ...
     */
    public get_label(): HTMLLabelElement | null {
        return this.label;
    }
     ...
     */
    public append_to(container: HTMLElement) {
        if (this.has_label()) {
...
class Task_Definition_Editor {
     ...
     * @type {string}
     */
    private static TASK_DEFINITION_EDITOR_CONTAINER_ID = "task-definition-editor-container";
     ...
     */
    public static async open(task_definition_id?: number) {
     ...
     */
    private static create_task_definition_editor_form(task_definition_id?: number, task_definition_data?: any): HTMLFormElement {

        const form = $("<form>")[0];

        // title field
        const title_field = new Form_Field("title", $, {
            default: task_definition_id ? task_definition_data.title : undefined
        }, { name: "Title" });
        title_field.append_to(form);

        // start offset field
        const start_offset_field = new Form_Field("start_offset", $, {
            input_type: "number",
            default: task_definition_id ? task_definition_data.start_offset_in_days : undefined
        }, { name: "Start Offset" });
        $(start_offset_field.get_field()).attr("step", "1").attr("min", "0");
        start_offset_field.append_to(form);

        // finish offset field
        const finish_offset_field = new Form_Field("finish_offset", $, {
            input_type: "number",
            default: task_definition_id ? task_definition_data.finish_offset_in_days : undefined
        }, { name: "Finish Offset" });
        $(finish_offset_field.get_field()).attr("step", "1").attr("min", "0");
        finish_offset_field.append_to(form);

        // description field
        const description_field = new Form_Field("description", $, {
            tag_name: "textarea",
            default: task_definition_id ? task_definition_data.description : undefined
        }, { "name": "Description" });
        $(description_field.get_field()).attr("cols", "100").attr("rows", "10");
        description_field.append_to(form);

        // task_definition_id field
        if (task_definition_id) {
            const id_field = new Form_Field("task_definition_id", $, { "default": task_definition_id.toString() });
            $(id_field.get_field()).hide();
            id_field.append_to(form);
        }

        // submit buttion
        const submit_button = new Form_Field(task_definition_id ? "Save" : "Create", $, {
            "input_type": "submit",
            "linebreak": false,
     ...
     */
    public static async handle_task_definition_editor_form_submit(evt: SubmitEvent, event_type_id: number) {
        evt.preventDefault();

        const formData = new FormData($(`#${this.TASK_DEFINITION_EDITOR_CONTAINER_ID} > form`)[0]);
        const formObj = Object.fromEntries(formData);

        const submit_value = evt.submitter ? evt.submitter.getAttribute("value") : "Save";
        ...
        switch (submit_value) {
            ...
            case 'Delete':
                ...
                Task_Definition_Requests.remove_task_definition(event_type_id, Util.parseID(formObj["task_definition_id"].toString()));
                this.exit();
                Task_Definition_Table.draw_editable(
                    event_type_id
                );
            ...
            case 'Save':
                ...
                Task_Definition_Requests.update_task_definition(event_type_id, Util.parseID(formObj["task_definition_id"].toString()), JSON.stringify(formObj));
                this.exit();
                Task_Definition_Table.draw_editable(
                    event_type_id
                );
            ...
            case 'Create':
                ...
                await Task_Definition_Requests.create_task_definition(event_type_id, JSON.stringify(formObj));
                this.exit();
                Task_Definition_Table.draw_editable(
                    event_type_id
                );
     ...
     */
    private static async draw(event_type_id: number, task_definition_id?: number) {
        ...
        // create title
        $(`<h2>${task_definition_id ? "Edit" : "Create"} Task Definition</h2>`).appendTo($div);

        // get task definition data
        var task_definition_data = task_definition_id ? (await (await Task_Definition_Requests.get_task_definition(event_type_id, task_definition_id)).json()).data : null;

        // create form
        const form = this.create_task_definition_editor_form(task_definition_id, task_definition_data);
        $div.append(form);
        $(form).on("submit", function (evt: any) {
            Task_Definition_Editor.handle_task_definition_editor_form_submit(evt.originalEvent, Event_Type_Editor_Page.get_event_type_id());
        });
    }
}
...
class Task_Definition_Table {
     ...
     * @type {string}
     */
    private static TASK_DEFINITIONS_CONTAINER_ID?: string;
     ...
     */
    private static get_task_definition_id_from_button_click(evt: MouseEvent): number {

        if (!(evt.target instanceof HTMLButtonElement)) {
            throw new Error("Failed to get task definition id: Element clicked was not button.")
        }
     ...
     * @returns {number} The task definition id.
     */
    private static get_task_definition_id_from_button(btn: HTMLButtonElement): number {

        if (!(btn.parentElement instanceof HTMLTableCellElement) ||
            !(btn.parentElement.parentElement instanceof HTMLTableRowElement)) {
            throw new Error("Failed to get task definition id: Page layout for table button is incorrect.");
        }

        let task_definition_id = btn.parentElement.parentElement.getAttribute("task_definition_id");
        if (task_definition_id === null) {
            throw new Error("Failed to get task definition id: Table row has no attribute task_definition_id");
        }
     ...
     */
    public static set_container_id(container_id: string) {
     ...
     */
    public static get_container_id(): string {
        if (!this.TASK_DEFINITIONS_CONTAINER_ID) {
            throw new Error("Task Definition Container ID undefined: try setting the id with set_container_id()");
        }
     ...
     */
    private static get_task_definitions_container(): HTMLDivElement {
     ...
     */
    public static async draw_editable(event_type_id: number) {

        const task_definition_data = await (await Task_Definition_Requests.get_task_definitions(event_type_id)).json();
        const container = this.get_task_definitions_container();

        const table = new Table();
        table.add_column("id", (row_datum) => { return row_datum.id });
        table.add_column("Title", (row_datum) => { return row_datum.title });
        table.add_column("Edit", () => {
            return $("<button>").on("click", function (evt: PointerEvent) {
                evt.preventDefault();
                if (!(evt.target instanceof HTMLButtonElement)) {
                    throw new Error("Click event wasn't triggered from button!");
                }
                Task_Definition_Editor.open(Task_Definition_Table.get_task_definition_id_from_button(evt.target));
            }).html("Edit")[0];
        });
        table.add_column("Remove", () => {
            return $("<button>").on("click", function (evt) { Task_Definition_Table.remove_task_definition(evt) }).html("Remove")[0];
        });
     ...
     */
    public static async remove_task_definition(evt: PointerEvent) {

        evt.preventDefault();

        const event_type_id = Event_Type_Editor_Page.get_event_type_id();
        const task_definition_id = this.get_task_definition_id_from_button_click(evt);

        Task_Definition_Requests.remove_task_definition(event_type_id, task_definition_id);
...
class Task_Definition_Requests {
     ...
     * @type {string}
     */
    private static API_NAMESPACE: string = '/wp-json/caataskplugin/v1';
     ...
     */
    public static async get_task_definition(event_type_id: number, task_definition_id: number): Promise<Response> {

        const response = await fetch(window.location.origin + this.API_NAMESPACE + `/event-types/${event_type_id}/task-definitions/${task_definition_id}`, {
            method: "GET"
        });
        if (!response.ok) {
            throw new Error(`Failed to retrieve task definitions (${response.status})`);
        }
     ...
     */
    public static async get_task_definitions(event_type_id: number): Promise<Response> {

        const response = await fetch(window.location.origin + this.API_NAMESPACE + `/event-types/${event_type_id}/task-definitions`, {
            method: "GET"
        });
        if (!response.ok) {
            throw new Error(`Failed to retrieve task definitions (${response.status})`);
     ...
     */
    public static async remove_task_definition(event_type_id: number, task_definition_id: number): Promise<Response> {
     ...
     */
    public static async update_task_definition(event_type_id: number, task_definition_id: number, data: any): Promise<Response> {
     ...
     */
    public static async create_task_definition(event_type_id: number, data: any): Promise<Response> {
...
class Event_Type_Requests {
     ...
     */
    public static async add_subtype(parent_event_type_id: number, subtype_id: number): Promise<Response> {

        const response = await fetch(window.location.origin + this.API_NAMESPACE +`/event-types/${parent_event_type_id}/subtypes`, {
            "method": "POST",
     ...
     * @returns {Promise<Response>} the response to the DELETE request
     */
    public static async remove_subtype(parent_event_type_id: number, subtype_id: number): Promise<Response> {

        const response = await fetch(window.location.origin + this.API_NAMESPACE + `/event-types/${parent_event_type_id}/subtypes/${subtype_id}`, {
            method: "DELETE"
        });
        if (!response.ok) {
     ...
     */
    public static async get_subtypes(parent_event_type_id: number): Promise<Response> {

        const response = await fetch(window.location.origin + this.API_NAMESPACE + `/event-types/${parent_event_type_id}/subtypes?addable=true&removable=true`, {
            method: "GET",
            headers: {
            }
        });
...
class Subtype_Editor {
    ...
    /**
     * The id for the container for the table listing addable subtypes.
     * @since 1.0.0
     * @type {string}
     */
    private static addable_subtypes_container_id?: string;

    /**
     * The id of the container for the table listing removable subtypes.
     * @since 1.0.0
     * @type {string}
     */
    private static removable_subtypes_container_id?: string;
     ...
     */
    public static async draw(event_type_id: number) {

        const event_type_data = await (await Event_Type_Requests.get_subtypes(event_type_id)).json();
     ...
     */
    public static async draw_addable_subtype_table(addable_subtypes: any[]) {

        const table: Table = new Table();
        table.add_column("Name", function (row_datum: any) {
            return row_datum.display_name;
        });
        table.add_column("Description", function (row_datum: any) {
            return row_datum.description;
        });
        table.add_column("Add", function (row_datum: any) {
            return $("<button>").attr("type", "button").on("click", function (evt: PointerEvent) {
                evt.preventDefault();
                if (!(evt.target instanceof HTMLButtonElement)) {
                    throw new Error("Pointer Event wasn't triggered by button press!");
                }
                Subtype_Editor.add_event_type(Subtype_Editor.get_subtype_id_from_button(evt.target))
            }).html("Add")[0];
        });
        table.set_row_callback(function (row_datum, rowEl) {
        ...
        table.draw(
            this.get_addable_subtypes_container(),
            addable_subtypes
        );
    }
     ...
     */
    public static async add_event_type(subtype_id: number) {
     ...
     */
    public static async draw_removable_subtype_table(removable_subtypes: any[]) {
        const table: Table = new Table();
        table.add_column("Name", function (row_datum: any) {
            return row_datum.display_name;
        });
        ...
        table.add_column("Remove", function (row_datum: any) {
            return $("<button>").attr("type", "button").on("click", function (evt: PointerEvent) {
                evt.preventDefault();
                if (!(evt.target instanceof HTMLButtonElement)) {
                    throw new Error("Pointer Event wasn't triggered by button press! ");
                }
                Subtype_Editor.remove_event_type(Subtype_Editor.get_subtype_id_from_button(evt.target))
            }).html("Remove")[0];
        });
        table.set_row_callback(function (row_datum, rowEl) {
            $(rowEl).attr("event_type_id", row_datum.id);
        ...
        table.draw(
            this.get_removable_subtypes_container(),
            removable_subtypes
        );
    }
     ...
     */
    public static async remove_event_type(subtype_id: number) {

        const event_type_id = Event_Type_Editor_Page.get_event_type_id();
        const res = await Event_Type_Requests.remove_subtype(event_type_id, subtype_id);
     ...
     */
    public static set_container_ids(addable_subtypes_container_id: string, removable_subtypes_container_id: string) {
     ...
     */
    private static get_container(container_id?: string): HTMLDivElement {
        if (!container_id) {
            throw new Error("The Container ID was never set, please use 'set_container_ids' .");
        }
        const container = document.getElementById(container_id);
        if (!container) {
            throw new Error("The Container ID does not match any existing elements.");
        } else if (!(container instanceof HTMLDivElement)) {
            throw new Error("The matching element to the container id is not a div element.");
        }
        return container;
    }
     ...
     */
    private static get_addable_subtypes_container(): HTMLDivElement {
     ...
     */
    private static get_removable_subtypes_container(): HTMLDivElement {
     ...
     */
    private static get_subtype_id_from_button(btn: HTMLButtonElement): number {

        if (!(btn.parentElement instanceof HTMLTableCellElement) ||
            !(btn.parentElement.parentElement instanceof HTMLTableRowElement)) {
            throw new Error("Failed to get subtype id: Page layout for table button is incorrect.");
        }

        const event_type_id_str: string | null = btn.parentElement.parentElement.getAttribute("event_type_id");
        if (null === event_type_id_str) {
            throw new Error("Failed to get subtype id: missing attribute 'event_type_id' on table row.");
        }
...
class Event_Type_Editor_Page {
     ...
     */
    public static set_container_ids(addable_subtypes_container_id: string, removable_subtypes_container_id: string, task_definitions_container_id: string) {
        Subtype_Editor.set_container_ids(addable_subtypes_container_id, removable_subtypes_container_id);
     ...
     */
    public static get_event_type_id(): number {

        let event_type_id_match = window.location.search.match(/(?<=event-type-id=)([^&]*)/);
...
class Event_Type_Manager_Page {
    ...
    /**
     * The id of the container where the event type table should be listed.
     * @since 1.0.0
     * @type string
     */
    private static event_type_container_id?: string;

    /**
     * Draws the table of event types.
     * @since 1.0.0
     */
    public static async draw_event_type_table() {

        const table = new Table();
        table.add_column("ID", function (row_datum: any) {
            return row_datum.id;
        });
        table.add_column("Name", function (row_datum: any) {
            return row_datum.display_name;
        ...
        table.add_column("Edit", function (row_datum: any) {
            return $("<a>").attr("href", window.location.href + "&action=edit&event-type-id=" + row_datum.id).html("Edit")[0];
        });

        const event_type_table_container: HTMLDivElement = this.get_event_type_container();
        const event_types_data = await (await Event_Type_Requests.get_event_types()).json();
     ...
     * will be drawn.
     */
    public static set_event_type_container_id(event_type_container_id: string) {
     ...
     */
    private static get_event_type_container(): HTMLDivElement {
        if (!this.event_type_container_id) {
            throw new Error("The Event Type Container ID was never set, please use 'set_event_type_container_id' .");
        }
        const container = document.getElementById(this.event_type_container_id);
        if (null === container) {
            throw new Error("The Event Type Container ID does not match any existing elements.");
        } else if (!(container instanceof HTMLDivElement)) {
            throw new Error("The matching element to the event type container id is not a div element.");
        }
        return container;
    }
